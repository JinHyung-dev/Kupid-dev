<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.util.List,com.kupid.admin.inquiry.model.dto.Inquiry" %>
<%
...
<div class="inq-sec">
	...
	<%if(inq!=null){ %>
			...
			<tr>
				...
				<td><%=i.getWriter()%></td>
				<%if(i.isAnswer()){ %>
					<td><button onclick="location.assign('<%=request.getContextPath()%>/admin/answerinsert.do?no=<%=i.getInqNo()%>')">답변</button></td>
				<%}else{ %>
					<td><button onclick="location.assign('<%=request.getContextPath()%>/admin/answerupdate.do?no=<%=i.getInqNo()%>')">수정</button></td>
				<%} %>
			</tr>
			<%} 
		}else{%>
