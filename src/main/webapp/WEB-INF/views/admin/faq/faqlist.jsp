<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.util.List,com.kupid.admin.faq.model.dto.Faq" %>
<%
...
<div class="faq-sec">
		...
		<div>
	        ...
	        <div id="search-회원">
				<form action="<%=request.getContextPath()%>/admin/searchFaq.do">
		...
		<table class="table-size">
			...
			<%if(faqs!=null){ %>
				...
				<tr>
					...
					<td><%=f.getFaqTitle()%></td>
					<td><%=f.getFaqDate()%></td>
					<td><button onclick="location.assign('<%=request.getContextPath()%>/admin/faqupdate.do?no=<%=f.getFaqNo()%>')">수정</button></td>
			...
			<div class="btn"><button onclick="location.assign('<%=request.getContextPath()%>/admin/faqinsert.do')">작성</button></div>
			<br><br><br>
			<div><%=pagebar%></div>
		</div>
</div>
	...
	<script>
		const deleteFaq=(n)=>{
			if(confirm("정말 삭제 하시겠습니까?")){
				location.assign("<%=request.getContextPath()%>/admin/faqdelete.do?no="+n)
			}else{
				alert("삭제가 취소되었습니다");
			}
		}
