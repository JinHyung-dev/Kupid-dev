		...
		<h1>
			<div>
	        	...
	        	<div id="search-member_name">
					<form action="<%=request.getContextPath()%>/admin/searchPenalty.do">
		...
		<table class="table-size">
			...
			<%if(member!=null){ %>
				...
				<tr>
					...
					<td><button onclick="location.assign('<%=request.getContextPath()%>/admin/memberview.do?no=<%=m.getMemberNo()%>')">상세정보</button></td>
...
const deleteMember=(n)=>{
	if(confirm("정말 삭제 하시겠습니까?")){
		location.assign("<%=request.getContextPath()%>/admin/memberdelete.do?no="+n);
	}else{
		alert("삭제가 취소되었습니다");
	}
}
