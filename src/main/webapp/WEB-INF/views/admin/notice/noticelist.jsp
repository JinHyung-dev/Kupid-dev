<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.util.List,com.kupid.admin.notice.model.dto.Notice" %>
<%
...
<%@ include file="/WEB-INF/views/admin/adminaside.jsp" %>
	<div class="notice-sec">
		...
		<h1>
			<div>
	        	...
	        	<div id="search-일반">
					<form action="<%=request.getContextPath()%>/admin/searchNotice.do">
		...
		<table class="table-size">
			...
			<%if(notices!=null){ %>
				...
				<tr>
					...
					<td><button onclick="location.assign('<%=request.getContextPath()%>/admin/noticeupdate.do?no=<%=n.getNoticeNo()%>')">수정</button></td>
			...
			<div class="btn"><button onclick="location.assign('<%=request.getContextPath()%>/admin/noticeinsert.do')">작성</button></div>
			<br><br>
			<div><%=pagebar%></div>
	</div>
</div>
	<script>
		const deleteNotice=(n)=>{
			if(confirm("정말 삭제 하시겠습니까?")){
				location.assign("<%=request.getContextPath()%>/admin/noticedelete.do?no="+n)
			}else{
				alert("삭제가 취소되었습니다");
			}
		}
